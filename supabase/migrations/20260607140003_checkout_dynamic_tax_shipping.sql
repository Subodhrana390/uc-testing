-- Migration to add dynamic tax and shipping capabilities to the checkout system
-- 1. Add base_charge to delivery_zones
ALTER TABLE public.delivery_zones ADD COLUMN IF NOT EXISTS base_charge numeric DEFAULT 50;

-- 2. Add tax_amount and shipping_amount to orders
...
-- 3. Replace place_order_safe RPC to calculate dynamic tax and shipping
...
BEGIN
    ...
    -- 3. Calculate Total, Tax, and Reserve Inventory
    FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items) AS x(id uuid, quantity int) LOOP
        -- Lock the product row
        SELECT price, stock_quantity, name, tax_rate INTO v_product
        ...
        v_tax_amount := v_tax_amount + ((v_product.price * v_item.quantity) * (COALESCE(v_product.tax_rate, 0) / 100.0));
    END LOOP;

    -- 4. Calculate Delivery Charge
    IF p_postal_code IS NOT NULL THEN
        -- First check pincodes override table
        SELECT zone_id INTO v_zone_id FROM public.delivery_pincodes WHERE pincode = p_postal_code AND active = true LIMIT 1;
        
        IF v_zone_id IS NULL THEN
            -- Check coverage prefix
            SELECT id, base_charge INTO v_zone_id, v_delivery_charge
            FROM public.delivery_zones 
            WHERE active = true 
            AND p_postal_code LIKE ANY(string_to_array(replace(coverage, ' ', ''), ',')) 
            LIMIT 1;
        ELSE
            -- We found an override zone, get its charge
            SELECT base_charge INTO v_delivery_charge FROM public.delivery_zones WHERE id = v_zone_id;
        END IF;
    END IF;
    
    -- Ensure fallback is safe
    IF v_delivery_charge IS NULL THEN
        v_delivery_charge := 50;
    END IF;

    -- 5. Apply Tax, Delivery Charge & COD Limit
    v_total_amount := v_total_amount + v_tax_amount + v_delivery_charge;
    ...
    -- 8. Insert Initial Status History
    ...
    RETURN jsonb_build_object('success', true, 'order_id', v_order_id, 'total_amount', v_total_amount, 'tax_amount', v_tax_amount, 'shipping_amount', v_delivery_charge);
