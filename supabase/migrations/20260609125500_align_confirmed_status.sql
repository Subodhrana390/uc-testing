-- Migration to align ORDER_CONFIRMED to CONFIRMED

-- 1. Update existing order statuses if any are ORDER_CONFIRMED
UPDATE public.orders SET status = 'CONFIRMED' WHERE status = 'ORDER_CONFIRMED';

-- 2. Update place_order_safe to use 'CONFIRMED' instead of 'ORDER_CONFIRMED'
...
-- 3. Update transition_order_status to use 'CONFIRMED' instead of 'ORDER_CONFIRMED'
...
BEGIN
  ...
  -- Normalize ORDER_CONFIRMED to CONFIRMED
  ...
  CASE v_current_status
    ...
    WHEN 'PAYMENT_SUCCESS', 'PLACED' THEN v_is_valid := p_new_status IN ('CONFIRMED', 'CANCELLED', 'REFUNDED');
    WHEN 'PAYMENT_FAILED', 'FAILED' THEN v_is_valid := p_new_status IN ('CANCELLED');
    WHEN 'CONFIRMED' THEN v_is_valid := p_new_status IN ('PROCESSING', 'CANCELLED');
